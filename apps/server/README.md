...
- 유저는 유저 프로필을 설정 할 수 있으며, 그룹별로 별도 프로필을 설정 할 수 있다.
- 유저/멤버 프로필은 `유저네임`, `프로필 이미지`로 이루어져 있다.
- `유저 프로필`의 유저네임이 설정되지 않았다면, 유저이름을 설정하도록 안내한다.
- `유저 프로필`의 유저네임이 없이 본 서비스를 이용 할 수 없다.
- 그룹에 들어갈 때 프로필과 유저네임을 설정 할 수 있으며, 이를 `멤버 프로필`이라 칭한다.
...
- 그룹을 생성한 멤버를 `owner`라 칭한다.
- 그룹에서 상호작용 하는 유저를 `member`라 칭한다.
- 오너 또한 멤버라 칭할 수 있다.
...
- 그룹은 최대 5개로 한다.
- 유저는 스스로 그룹에서 탈퇴 할 수 있다.
- 오너는 그룹을 탈퇴 할 수 없다.
  - 그룹 탈퇴를 위해서는 오너의 자리를 양도해야 한다.
...
- user: 일반 사용자
  - user profile : username, profileImageUrl 로 이루어졌으며, 유저의 기본 정보이다.
  - member profile : user profile과 동일한 데이터 구조이나, 특정 group 내에서 사용 할 profile 이다.
- group: 그룹 앨범
